...
const sortImmutableTyped = (arr: TypedArray): number[] => {
  // Micro-optimizations, but forgivable
  // as speed is what we're after, here.
  let iter1: number = 0;
  let iter2: number = 0;
  // See reduceTyped.ts for information about this.
  const ArrConstructor = arr.constructor as TypedArrayConstructor;
  const tempIndices = new ArrConstructor(arr.length);
...
export default sortImmutableTyped;
